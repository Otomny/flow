...
public class MongoDBRepository<T, ID>
    implements MongoRepository<T, ID>, ServerInfo {

  public static final ReplaceOptions UPSERT_OPTIONS =
  ...
  public MongoDBRepository(Class<T> dataClass, Class<ID> idClass,
    ...
    this.fieldUpdater = (fieldData) -> {
      dispatcher.submit(() -> {
        ...
        Codec<Object> codec =
            (Codec<Object>)codecs.getCodecRegistries().get(field.getType());
        ...
        ObjectUpdate update =
            new ObjectUpdate(id.toString(), this.collectionName,
    ...
    this.topic.addListener(ObjectUpdate.class, (channel, objectUpdate) -> {
      ...
      try {
        Field field =
            this.dataClass.getDeclaredField(objectUpdate.getFieldName());
        ...
        BsonDocumentReader dReader =
            new BsonDocumentReader(container.toBsonDocument());
  ...
  public void deleteAllById(Iterable<? extends ID> ids) {
    throw new UnsupportedOperationException(
        "Delete all by id is not implemented");
  }
  ...
  public Optional<T> findById(ID id) {
    if (this.cachedData.containsKey(id)) {
      try {
        return Optional.of(ProxyMongoObject.createProxy(this.cachedData.get(id),
                                                        this.fieldUpdater));
      } catch (Exception e) {
    ...
    try {
      return Optional.of(
          ProxyMongoObject.createProxy(object, this.fieldUpdater));
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
  ...
  public Iterable<T> findAllById(Iterable<ID> ids) {
    ...
    for (ID id : ids) {
      if (this.cachedData.containsKey(id)) {
        availableObjets.add(ProxyMongoObject.createProxySilent(
            this.cachedData.get(id), this.fieldUpdater));
      } else {
        notFoundInCache.add(id);
      }
    }
    var entitiesFound = this.collection.find(
        Filters.in("_id", notFoundInCache.stream().map(ID::toString).toList()));

    for (T object : entitiesFound) {
      var id = this.getId.apply(object);
      this.cachedData.put(id, object);
      availableObjets.add(
          ProxyMongoObject.createProxySilent(object, this.fieldUpdater));
    }
    return availableObjets;
  }
  ...
  public <S extends T> boolean save(S entity) {
    ...
    if (isProxy(entity)) {
      ...
      var result = this.collection.replaceOne(filter, this.cachedData.get(id),
                                              UPSERT_OPTIONS);
  ...
  public List<Document> executeQuery(Bson filter, Bson projection) {
    return StreamSupport
        .stream(
            this.collection.find(filter, Document.class).projection(projection).spliterator(),
            false)
        .toList();
  }
}
