...
public class Area {
  ...
  @OField private Location start;
  @OField private Location end;
    ...
    for (int y = maxY; y >= minY; y--) {
      for (int x = minX; x <= maxX; x++) {
        for (int z = minZ; z <= maxZ; z++) {
          ...
          BlockState state =
              chunk.getBlock(NumberUtils.mod(x, 16), y, NumberUtils.mod(z, 16))
                  .getState();
  ...
  public CompletableFuture<List<BlockUpdate>>
  getBlockUpdate(Dispatcher dispatcher) {
    Objects.requireNonNull(this.start, "Error: Area, this.start is null");
    Objects.requireNonNull(this.end, "Error: Area, this.end is null");
