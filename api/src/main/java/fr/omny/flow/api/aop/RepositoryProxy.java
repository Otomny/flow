...
public class RepositoryProxy implements InvocationHandler {
	...
	public static <T, ID, C extends CrudRepository<T, ID>> C createRepositoryProxy(Class<? extends C> proxyClass,
		...
		return (C) Proxy.newProxyInstance(proxyClass.getClassLoader(), new Class[] {
				proxyClass }, new RepositoryProxy(proxyClass, crudRepository));
	}

	private CrudRepository<?, ?> repo;
	private Class<?> proxiedClass;

	private RepositoryProxy(Class<?> proxiedClass, CrudRepository<?, ?> crudRepository) {
	...
	public Object invoke(Object proxy, Method method, Object[] arguments) throws Throwable {
		...
		for (Method remoteMethod : repo.getClass().getDeclaredMethods()) {
			if (remoteMethod.getParameterCount() == argumentsCount && remoteMethod.getName().equals(methodName)) {
		...
		return MethodHandles
				.lookup().findSpecial(proxiedClass, method.getName(),
						MethodType.methodType(method.getReturnType(), parametersType), proxiedClass)
